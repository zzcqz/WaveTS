...
class MSRAblock(nn.Module):
    ...
    def forward(self, x):
        ...
        h = x1+x2+x3+x4
        h = self.se(h)
        x = self.convx(x)   #scale = (batchsize,321,len)
        x = self.relu(x)
        out = x + h
        return out
...
class Model(nn.Module):
    ...
    def __init__(self, configs):
        ...
        self.msr = MSRAblock()
        self.conv1 = nn.Conv1d(in_channels=8 , out_channels= 8, kernel_size=3, stride=1,dilation=1,padding=1)
        self.conv2 = nn.Conv1d(in_channels=8 , out_channels= 8, kernel_size=1, stride=1,dilation=1,padding=0)
        self.relu = nn.ReLU()
